...
switch(cl_dlsis_state)
{
		...
		switch (cl_dlsis_event)
			{
				...
				if(Cl_ConsoleRxMsg.msgready == true)
				{
					Cl_Dlsis_Get_data(Cl_ConsoleRxMsg.data.byte[0],  1);			
					...
					if(Cl_dlsisretcode == CL_OK)
						{
							...
							cl_dlsis_state = CL_DLSIS_STATE_BO1_V13V14_TRANSITION;
							...
							Cl_dlsisretcode =  Cl_AlarmActivateAlarms( HOLDER1STATUS_OPEN,true );
							Cl_dlsisretcode =  Cl_AlarmActivateAlarms( HOLDER2STATUS_OPEN,true );
							...
							Cl_dlsisretcode = sv_cntrl_setflowpath(CL_DLSIS_STATE_BO1_V13V14_TRANSITION);
							Cl_dlsisretcode = SetHeaterState(CL_HEATER_STATE_START);
		...
		case CL_DLSIS_STATE_BO1_V13V14_TRANSITION:
				switch(cl_dlsis_event)
				{
		...
		switch(cl_dlsis_event)
			{
				...
				{
					Cl_Alarm_GetAlarmStatus(PS3STATUS_HIGH,&alarmstatus1);
				}
				...
				if(alarmstatus1 )
				{
					...
					{
						Cl_dlsisretcode =  sv_cntrl_deactivatepump(DCMOTOR1);
						Cl_dlsisretcode =  sv_cntrl_deactivatepump(DCMOTOR2);
					}
					if(Cl_dlsisretcode == CL_OK)
					{
						cl_dlsis_state = CL_DLSIS_STATE_BO2_V13V14_TRANSITION;
						bcs_wait = false;
		...
		break;
		case CL_DLSIS_STATE_BO2_V13V14_TRANSITION:
				switch(cl_dlsis_event)
				{
...
Cl_ReturnCodes Cl_Dlsis_StopDialysis(void)
{
	...
	Cl_ReturnCodes Cl_dlsisretcode = CL_OK;
				...
				//Cl_dlsisretcode = Cl_SendDatatoconsole(CON_TX_COMMAND_RINSE_STOPPED,&data,0);
...
//	Cl_dlsisretcode = (uint8_t)sv_nvmgetdata(NV_DIALYSATE_FLOW, &tempdata);
	...
	systemdataarray[count++] = 0xAA; // tmp
	systemdataarray[count++] = 0xAA; // tmp
	
	systemdataarray[count++] = 0xBB;
	systemdataarray[count++] = 0xBB;
...
Cl_ReturnCodes	Cl_Dlsis_StartDialysis(void)
{
	...
	Cl_dlsisretcode = SetHeaterState(CL_HEATER_STATE_START);
	//sv_cntrl_activatepump(BLOODPUMP);
	//cl_bp_controller(CL_BP_EVENT_SET_BP_RATE, 2000);
